package com.company;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class Parallelogram implements GeometricPrimitive {
    private double a;
    private double b;
    private double h;

    public Parallelogram() {

    }

    public Parallelogram(double a, double b, double h) {
        this.a = a;
        this.b = b;
        this.h = h;
    }

    public double getA() {
        return this.a;
    }

    public void setA(double a) {
        this.a = a;
    }

    public double getB() {
        return this.b;
    }

    public void setB(double b) {
        this.b = b;
    }

    public double getH() {
        return this.h;
    }

    public void setH(double h) {
        this.h = h;
    }

    @Override
    public double getPerimeter() {
        return 2 * (a + b);
    }

    @Override
    public double getArea() {
        return b * h;
    }

    // proper format for serialization:
    // <geometric-primitive-type> <field1>=<value1> <field2>=<value2>
    public void serialize(FileWriter fw) {
        try (BufferedWriter bw = new BufferedWriter(fw)) {

            // Compose a String according to the format
            String res = this.getClass().getSimpleName().toLowerCase() + " " + "a=" + this.a + " " + "b=" + this.b + " "
                    + "h=" + this.h;

            // Perform the write
            bw.write(res);

        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    }

    static Parallelogram deserialize(FileReader fr) throws IOException {
        Parallelogram tempParallelogram = new Parallelogram();
        try (BufferedReader br = new BufferedReader(fr)) {

            // Read the line
            String res = br.readLine();

            // Split the line
            String[] splitted = res.split("\\s+");

            // Check if the format is proper
            if (splitted[0].equals("parallelogram") && splitted[1].startsWith("a") && splitted[2].startsWith("b")
                    && splitted[3].startsWith("h")) {

                // Get a and set tempParallelogram's a to the value found
                double a = Parallelogram.findDouble(splitted[1]);
                tempParallelogram.setA(a);

                // Get b and set tempParallelogram's b to the value found
                double b = Parallelogram.findDouble(splitted[2]);
                tempParallelogram.setB(b);

                // Get h and set tempParallelogram's h to the value found
                double h = Parallelogram.findDouble(splitted[3]);
                tempParallelogram.setH(h);

                // throw an IOException if the format is wrong
            } else {
                throw new IOException(
                        "Invalid format! Proper format: <geometric-primitive-type> <field1>=<value1> <field2>=<value2>");
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
        return tempParallelogram;
    }

    private static double findDouble(String source) {
        // Takes a String as an argument and finds a double in the provided String
        String temp = "";
        Matcher m = Pattern.compile("\\d+\\.\\d+").matcher(source);
        while (m.find()) {
            temp = m.group();
        }
        double value = Double.parseDouble(temp);
        return value;
    }

}
